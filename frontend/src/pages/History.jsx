...
import { HiSearch, HiDownload, HiEye, HiTrash, HiChevronLeft, HiChevronRight } from 'react-icons/hi';
...
const History = () => {
  ...
  const filteredQRs = qrCodes.filter(qr => {
    const matchesSearch = qr.text?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleView = (qr) => {
    // Open QR code in a new tab/window
    window.open(qr.qrImage, '_blank');
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">QR Code History</h1>
          ...
          {filteredQRs.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500 dark:text-gray-400">
                {qrCodes.length === 0 ? 'No QR codes found. Generate your first QR code!' : 'No QR codes match your filters.'}
              </p>
            </div>
          ) : viewMode === 'table' ? (
                ...
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700">
                    <th className="text-left py-3">
                      <input
                        type="checkbox"
                        checked={selectedQRs.length === paginatedQRs.length && paginatedQRs.length > 0}
                        onChange={handleSelectAll}
                        className="rounded"
                      />
                    </th>
                    <th className="text-left py-3 font-medium text-gray-900 dark:text-white">Preview</th>
                    <th className="text-left py-3 font-medium text-gray-900 dark:text-white">Content</th>
                    <th className="text-left py-3 font-medium text-gray-900 dark:text-white">Type</th>
                    <th className="text-left py-3 font-medium text-gray-900 dark:text-white">Created</th>
                    <th className="text-left py-3 font-medium text-gray-900 dark:text-white">Scans</th>
                    <th className="text-left py-3 font-medium text-gray-900 dark:text-white">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedQRs.map((qr) => (
                    <tr key={qr._id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                      <td className="py-4">
                        <input
                          type="checkbox"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {paginatedQRs.map((qr) => (
                <div key={qr._id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    <p className="text-sm text-gray-900 dark:text-white truncate" title={qr.text}>
                      {qr.text}
                    </p>
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center mt-6">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredQRs.length)} of {filteredQRs.length} results
              </p>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              onClick={() => handleDelete(qrToDelete)}
              variant="danger"
            >
              Delete
            </Button>
